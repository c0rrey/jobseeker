...
def _update_company(
    ...
    """Write enrichment fields into the companies table row for company_name.

    Args:
        conn: Open SQLite connection with the V2 schema.
        company_name: Used to locate the row via the ``name`` column.
...
def enrich(company_name: str, db_connection: sqlite3.Connection) -> bool:
    ...
    to the companies table row whose ``name`` matches.
